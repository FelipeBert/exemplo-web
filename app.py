...
def login():
    ...
    with sqlite3.connect("database.db") as users:
        cursor = users.cursor()
        query = "SELECT * FROM PARTICIPANTS WHERE (username = (?) OR email = (?) ) AND password = (?)"
        cursor.execute(query, (email_or_username, email_or_username, (hashlib.md5(password.encode("utf-8")).hexdigest() ) ) ) 
        result = cursor.fetchall()
